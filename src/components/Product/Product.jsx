...
class Product extends Component {
  ...
  fetchProduct = () => {
    ...
    fetch("/products/myproduct/" + location[location.length - 1])
      .then(res => res.json())
      .then(json => {
        if (json.success) {
  ...
  findInLocal = () => {
    ...
    if (
      this.state.productArray.find(
        item => item._id.toString() === id[id.length - 1].toString()
      )
    ) {
      ...
      return (
        <div style={{ overflowX: "auto" }}>
          <Container>
            <div className="dib ma2 pa2 shadow-4 grow">
              <StyleRoot style={styles.fadeIn}>
                <img
                  ...
                  style={{ width: "500px", height: "700px", minWidth: "500px" }}
                />
              </StyleRoot>
            </div>
            ...
            <div>
              <ProductPanel description={this.state.description} />
            </div>
          </Container>
        </div>
      );
